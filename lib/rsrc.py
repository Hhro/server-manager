...
class ResourceManager:
    def __init__(self, is_debug=True):
        ...
        self.accounts_p = RESOURCES_DIR / "accounts.json"
        ...
        if not self._keys_dir.exists():
            self._keys_dir.mkdir(exist_ok=True, parents=True)

        if not self._creds_dir.exists():
            self._creds_dir.mkdir(exist_ok=True, parents=True)

        if not self.accounts_p.exists():
            self.accounts_p.touch()
            self.accounts_p.write_text("{}")
    ...
    def get_script_p(self, idx: int) -> Path:
        if idx < 1 or idx > len(self._scripts_p):
            return None
        return self._scripts_p[idx-1]
    ...
    def load_accounts(self) -> dict:
        with open(self.accounts_p, "r") as f:
            return json.load(f)
